...
import { 
  ChevronRight, Globe, Dna, Heart, Baby, 
  Sparkles, Activity, Menu, X, Info, 
  ArrowLeft, ArrowRight, Shield, Zap, Scissors, Terminal, HelpCircle, ShieldCheck, Activity as Pulse, AlertCircle
} from 'lucide-react';

const BiologySchematic = ({ detail }) => {
...
const BIOLOGY_DATA = {
  "Internal Medicine": { "icon": <Activity size={20} />, "color": "#ff4d4d", "items": [] },
  "Fundamentals": { "icon": <Globe size={20} />, "color": "#3498db", "items": [] },
  "Pediatrics": { "icon": <Baby size={20} />, "color": "#2ecc71", "items": [] },
  "Obstetrics & Gynecology": { "icon": <Activity size={20} />, "color": "#9b59b6", "items": [] },
  "Dermatology": { "icon": <Sparkles size={20} />, "color": "#f1c40f", "items": [] },
  "Surgery": { "icon": <Scissors size={20} />, "color": "#e67e22", "items": [] }
};

const ExploreBiology = ({ onBack }) => {
  const [tutorials, setTutorials] = useState([]);
  const [activeCategory, setActiveCategory] = useState("Internal Medicine");
  ...
  const filteredItems = tutorials.filter(t => {
    ...
    // Define other specific categories to exclude them from Fundamentals
    const otherSpecificCats = ["internal medicine", "pediatrics", "obstetrics & gynecology", "obstetrics and gynecology", "dermatology", "surgery"];
    ...
    const isDirectMatch = 
      ...
      (active.includes("obstetrics") && topicId.startsWith("obg")); // Special case for OBG prefix
    ...
    if (active === "fundamentals") {
      const matchesOther = otherSpecificCats.some(other => {
        const normOther = other.replace(/&/g, 'and').replace(/[^a-z0-9]/g, '');
  ...
  return (
    <div className="biology-pry-root min-vh-100 d-flex flex-column overflow-hidden position-relative">
      ...
      <header className="pry-header d-flex align-items-center justify-content-between px-4">
        <div className="d-flex align-items-center gap-3">
          <button 
            onClick={onBack}
            className="btn btn-link p-0 text-white opacity-50 hover-opacity-100 transition-all me-2"
          >
            <ArrowLeft size={20} />
          </button>
          <div className="pry-logo-box">
            <div className="pulse-indicator"></div>
            <Dna color="white" size={22} />
          </div>
          <div className="d-flex flex-column">
            <h5 className="m-0 fw-black text-white text-uppercase" style={{ letterSpacing: '-1px' }}>Explore Biology</h5>
        ...
        <div className="d-flex align-items-center gap-3 d-none d-md-flex">
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            <div>
              ...
              <span className="small text-white fw-bold">BIOLOGY_CORE_ENGINE</span>
            </div>
          </div>
        </div>
      ...
      <nav className="pry-top-nav px-4 d-flex align-items-center overflow-auto">
        <div className="d-flex gap-1 py-2">
          {Object.keys(BIOLOGY_DATA).map((cat) => (
            <motion.button
              key={cat}
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
            ...
            >
              <span style={{ opacity: activeCategory === cat ? 1 : 0.4 }}>
                {React.cloneElement(BIOLOGY_DATA[cat].icon, { size: 14 })}
              </span>
        ...
        <main className="content-area-pry flex-grow-1 overflow-auto p-3 p-md-4 position-relative">
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              >
                <BiologySchematic detail={activeDetail} />
              </motion.div>
            ) : (
