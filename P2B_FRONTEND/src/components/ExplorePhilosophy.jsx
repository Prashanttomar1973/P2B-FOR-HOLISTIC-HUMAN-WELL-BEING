...
import { 
  ChevronRight, BookOpen, Sparkles, Scale, Puzzle, Landmark, Globe,
  Menu, X, Info, ArrowRight, Shield, Zap, Terminal, HelpCircle, ShieldCheck, Activity, Brain, AlertCircle
} from 'lucide-react';

const PhilosophySchematic = ({ detail }) => {
...
const PHILOSOPHY_DATA = {
  "Metaphysics": { "icon": <Sparkles size={20} />, "color": "#00ffa2", "items": [] },
  "Epistemology": { "icon": <BookOpen size={20} />, "color": "#3498db", "items": [] },
  "Ethics": { "icon": <Scale size={20} />, "color": "#e74c3c", "items": [] },
  "Logic": { "icon": <Puzzle size={20} />, "color": "#f1c40f", "items": [] },
  "Political Philosophy": { "icon": <Landmark size={20} />, "color": "#9b59b6", "items": [] },
  "Fundamentals": { "icon": <Globe size={20} />, "color": "#2ecc71", "items": [] }
};

const ExplorePhilosophy = () => {
  const [tutorials, setTutorials] = useState([]);
  const [activeCategory, setActiveCategory] = useState("Metaphysics");
  ...
  const filteredItems = tutorials.filter(t => {
    ...
    // Define other specific categories for Philosophy
    const otherSpecificCats = ["metaphysics", "epistemology", "ethics", "logic", "political philosophy"];
    ...
    const isDirectMatch = 
      ...
      (active.includes("epistemology") && (
        topicId.startsWith("epi") || topicId.startsWith("phi-epi") ||
        topicName.includes("know") || topicName.includes("belief") || topicName.includes("truth") || 
        topicName.includes("skeptic") || topicName.includes("rational") || topicName.includes("empiric") ||
        topicName.includes("percept") || topicName.includes("certain") || topicName.includes("doubt") || 
        topicName.includes("evidence") || topicName.includes("plato") || topicName.includes("descartes") ||
        topicName.includes("kant") || topicName.includes("hume") || topicName.includes("locke") ||
        topicName.includes("reasoning") || topicName.includes("justification")
      )) ||
      (active.includes("ethics") && (
        topicId.startsWith("eth") || topicId.startsWith("phi-eth") ||
        topicName.includes("moral") || topicName.includes("virtue") || topicName.includes("utilitarian") || 
        topicName.includes("right") || topicName.includes("wrong") || topicName.includes("duty") || 
        topicName.includes("justice") || topicName.includes("aristotle") || topicName.includes("mill") ||
        topicName.includes("bentham") || topicName.includes("nietzsche") || topicName.includes("values") ||
        topicName.includes("freedom") || topicName.includes("good") || topicName.includes("evil")
      )) ||
      (active.includes("metaphysics") && (
        topicId.startsWith("meta") || topicId.startsWith("phi-met") ||
        topicName.includes("exist") || topicName.includes("reality") || topicName.includes("mind") || 
        topicName.includes("body") || topicName.includes("soul") || topicName.includes("time") ||
        topicName.includes("space") || topicName.includes("universe") || topicName.includes("god")
      )) ||
      (active.includes("logic") && (
        topicId.startsWith("log") || topicId.startsWith("phi-log") ||
        topicName.includes("argument") || topicName.includes("fallacy") || topicName.includes("reason") || 
        topicName.includes("valid") || topicName.includes("sound") || topicName.includes("proof") ||
        topicName.includes("deduct") || topicName.includes("induct")
      )) ||
      (active.includes("political") && (
        topicId.startsWith("pol") || topicId.startsWith("phi-pol") ||
        topicName.includes("state") || topicName.includes("power") || topicName.includes("law") || 
        topicName.includes("govern") || topicName.includes("liberty") || topicName.includes("equal") ||
        topicName.includes("social contract") || topicName.includes("democracy") || topicName.includes("rights")
      ));
  ...
  return (
    <div className="phil-pry-root min-vh-100 d-flex flex-column overflow-hidden position-relative">
      ...
      <header className="pry-header d-flex align-items-center justify-content-between px-4">
        <div className="d-flex align-items-center gap-3">
          ...
          <div className="d-flex flex-column">
            <h5 className="m-0 fw-black text-white text-uppercase" style={{ letterSpacing: '-1px' }}>Explore Philosophy</h5>
        ...
        <div className="d-flex align-items-center gap-3 d-none d-md-flex">
          ...
          <div className="d-flex align-items-center gap-2">
            ...
            <div>
              ...
              <span className="small text-white fw-bold">PHILOSOPHY_CORE_ENGINE</span>
            </div>
          </div>
        </div>
      ...
      <nav className="pry-top-nav px-4 d-flex align-items-center overflow-auto">
        <div className="d-flex gap-1 py-2">
          {Object.keys(PHILOSOPHY_DATA).map((cat) => (
            <motion.button
              key={cat}
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
            ...
            >
              <span style={{ opacity: activeCategory === cat ? 1 : 0.4 }}>
                {React.cloneElement(PHILOSOPHY_DATA[cat].icon, { size: 14 })}
              </span>
        ...
        <main className="content-area-pry flex-grow-1 overflow-auto p-3 p-md-4 position-relative">
          <AnimatePresence mode="wait">
            {loading ? (
              ...
              >
                <PhilosophySchematic detail={activeDetail} />
              </motion.div>
            ) : (
      ...
      {/* Content Footer */}
      <div className="content-footer-glass px-4 py-2 d-flex align-items-center justify-content-between">
        <div className="d-flex align-items-center gap-3">
          <span className="small fw-black opacity-30">SECURE SIGNAL [88-X]</span>
          <div className="signal-bars d-flex gap-1 align-items-end">
        ...
        <div className="small opacity-20 fw-bold">P2B ENGINE &copy; 2026</div>
      </div>
...
export default ExplorePhilosophy;
